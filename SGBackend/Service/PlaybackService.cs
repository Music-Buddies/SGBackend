...
public class PlaybackService
{
    ...
    public async Task<List<PlaybackRecord>> InsertNewRecords(List<SpotifyHistoryWithUser> histories)
    {
        ...
        foreach (var history in histories)
        {
            ...
            var latestPlaybackRecord = history.user.PlaybackRecords.OrderByDescending(record => record.PlayedAt)
                .FirstOrDefault();
    ...
    public async Task UpdateMutualPlaybackOverviews(List<PlaybackSummary> upsertedSummaries)
    {
        ...
        if (upsertedSummaries.Any(s => s.User != user)) throw new Exception("summaries of multiple users provided");

        if (user == null) throw new Exception("user must be included");
